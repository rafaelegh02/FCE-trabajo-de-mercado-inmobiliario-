...
# == EXPLORACION BASICA DE LAS BASES DE DATOS ====================================# 
...
df_venta <- df_venta %>%
  mutate(
    ...
    fecha = paste0(ano, "-Q", trimestre),                # Crea una columna de fecha con el anio y el trimestre             
...
df_prestamos <- df_prestamos %>%
  ...
  mutate(
    monto = as.numeric(monto),                                           # Convierte el monto a numérico
    fecha = ym(ano_mes)                                                  # Crea una fecha del 1 dia del mes y anio
  ) %>% select(fecha, monto)                                             # Se queda solo con las columnas fecha y monto
...
# Evolución del precio de venta (USD/m²)
